 ...
 *
 * Usage:
 *   bun run skill/adapters/edit/upload-source-text.ts <source_id> --file <path> [--provider transcript]
 *   bun run skill/adapters/edit/upload-source-text.ts <source_id> '{"raw_text":"...","provider":"transcript"}'
 */
...
if (!sourceId) {
  console.error("Usage: bun run skill/adapters/edit/upload-source-text.ts <source_id> (--file <path> | '<JSON payload>')");
  process.exit(1);
}
